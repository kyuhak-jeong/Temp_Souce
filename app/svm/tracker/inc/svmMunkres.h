#ifndef SVMMUNKRES_H_
#define SVMMUNKRES_H_
...
#define FORCE_QUIT_COUNT_THRESHOLD  (100000)

template<typename Data> class sanMunkres
{
...
public:
    ...
    void solve(Matrix<Data> &m) 
    {
        ...
        // Follow the steps
        int force_quit_count = 1; //changed by DGH
        int step = 1;
        while ( step && (force_quit_count++ < FORCE_QUIT_COUNT_THRESHOLD))
        {
            switch ( step ) 
            {
            case 1:
      ...
      for ( size_t row = 0 ; row < rows ; row++ ) 
      {
        for ( size_t col = 0 ; col < columns ; col++ ) 
        {
          if ( matrix(row, col) != infinity ) 
          {
            ...
            {
              max_value = std::max<Data>(max_value, matrix(row, col));
            }
          }
        }
      }
    ...
    static void minimize_along_direction(Matrix<Data> &matrix, const bool over_columns) 
    {
      ...
      {
        Data min_value = over_columns ? matrix(0, i) : matrix(i, 0);
        ...
        for ( size_t j = 1 ; j < inner_size && min_value > 0 ; j++ )
        {
            min_value = std::min<Data>(min_value, over_columns ? matrix(j, i) : matrix(i, j));
        }
