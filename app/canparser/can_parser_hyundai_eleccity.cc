...
static void can_parser_0CFDCCE6 (const uint8_t*, VehicleInfo_t &);
static void can_parser_18F0090B (const uint8_t*, VehicleInfo_t &);
static void can_parser_18FEF100 (const uint8_t*, VehicleInfo_t &);
static void can_parser_18FF2AEF (const uint8_t*, VehicleInfo_t &);
static void can_parser_18FEF1EF (const uint8_t*, VehicleInfo_t &);

#define REGISTER_CAN_PARSER_ID(can_id) m_parser_map.emplace(0x##can_id, &can_parser_##can_id);

AIVCANParserHyundaiElecCity::AIVCANParserHyundaiElecCity(const char* dev_name): AIVCANParser(dev_name)
{
...
AIVCANParserHyundaiElecCity::~AIVCANParserHyundaiElecCity()
{
}

void AIVCANParserHyundaiElecCity::getCanIdFilterCfg(std::vector<Cmd_CanBusFilterCfg_t> & cfg)
{
...
int AIVCANParserHyundaiElecCity::parse(const CanFrame_t &frm, VehicleInfo_t &veh_inf)
{
  int is_updated = 0;
